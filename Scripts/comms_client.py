...
class CommsClient:
    ...
    SDXL Workflows (IDs ending with -sdxl):
        ...
        The split between global (text_g) and local (text_l) is done on the server side.
    ...
    @staticmethod
    def http_free(server: str = DEFAULT_SERVER,
                  unload_models: bool = True, free_memory: bool = False) -> dict:
    ...
    @staticmethod
    def http_get_params(workflow_id: str, server: str = DEFAULT_SERVER) -> dict:
        """获取工作流当前参数 (默认 + 用户覆盖合并).
        ...
        Returns:
            {
                ...
                "name": "角色立绘设计",
                "defaults": {"steps": 20, "cfg": 7.0, ...},
                "overrides": {"steps": 30},
                "current": {"steps": 30, "cfg": 7.0, ...},
                "schema": {"steps": {"type":"int","default":20,"min":10,...}, ...}
            }
        """
        return CommsClient._http_request("GET", f"/workflows/{workflow_id}/params", server)

    @staticmethod
    def http_update_params(workflow_id: str, params: dict,
                           user: str = "anonymous", server: str = DEFAULT_SERVER) -> dict:
        """更新工作流参数 (自动验证+记录历史).

        Args:
            workflow_id: 工作流/蓝图ID
            params: 要更新的参数 dict, e.g. {"steps": 30, "cfg": 6.5}
            user: 操作者标识, 用于变更历史记录

        Returns:
            {
                "success": True/False,
                "message": "参数已保存" / "参数验证失败",
                "current": {...},
                "errors": [...]  (仅验证失败时)
            }

        Example:
            >>> r = CommsClient.http_update_params("game-character-design", {"steps": 30, "cfg": 6.0})
            >>> if r["success"]:
            ...     print(f"参数已更新: {r['current']}")
        """
        data = {"params": params, "user": user}
        return CommsClient._http_request("PUT", f"/workflows/{workflow_id}/params", server, data=data)

    @staticmethod
    def http_params_history(workflow_id: str, limit: int = 50,
                            server: str = DEFAULT_SERVER) -> dict:
        """获取参数变更历史.
        ...
        Returns:
            {
                "workflow_id": "game-character-design",
                "history": [
                    {"timestamp": "2026-06-16 15:30:00", "user": "dev", "params": {"steps": 30}},
                    ...
                ],
                "count": 3
            }
        """
        return CommsClient._http_request("GET", f"/workflows/{workflow_id}/params/history", server)
    ...
    @staticmethod
    async def get_params(workflow_id: str, server: str = DEFAULT_SERVER) -> dict:
        """Async: 获取工作流当前参数."""
        return CommsClient.http_get_params(workflow_id, server)

    @staticmethod
    async def update_params(workflow_id: str, params: dict,
                            user: str = "anonymous", server: str = DEFAULT_SERVER) -> dict:
        """Async: 更新工作流参数."""
        return CommsClient.http_update_params(workflow_id, params, user, server)

    @staticmethod
    async def params_history(workflow_id: str, limit: int = 50,
                             server: str = DEFAULT_SERVER) -> dict:
        """Async: 参数变更历史."""
        return CommsClient.http_params_history(workflow_id, limit, server)
