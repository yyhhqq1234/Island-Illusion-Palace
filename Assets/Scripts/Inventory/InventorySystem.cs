...
public class InventorySystem : MonoBehaviour, IInventoryService, IResourceCollector
{
    ...
    public bool AddMaterial(MaterialTypeEnum materialType, int quantity)
    {
        ...
        else
        {
            ...
            // 没有该材料，创建新物品
            GameSystems.MaterialData materialData = GetMaterialData(materialType);
    ...
    // 获取材料数据
    private GameSystems.MaterialData GetMaterialData(MaterialTypeEnum type)
    {
    ...
    {
        return MaterialDatabase.GetMaterialName(type);
    }
